 ...
 */
module.exports = { linkResolver, docResolver, getPathById, getParents, getImmediateChildren, getSiblings, reloadLinkmap, size }

function size () {
  return linkmap == null ? 0 : linkmap.size
}

function reloadLinkmap () {
  try {
    var a = fs.readFileSync('links.map')
    linkmap = new Map(JSON.parse(a))
  } catch (e) {
    console.error('[Resolve]: link map is missing')
  } finally {
    console.log('[Resolve]: reload link map', size())
  }
}
