 ...
 */
module.exports = function linkResolver (doc, db) {
...
function getPathRecursive (db, doc, result, n) {
  const link = db ? db.get(doc.default_uid) : false
  if (link) {
    console.log('db hit', doc.default_uid)
    return link
  }
  ...
  if (!item.data.parent_page || !item.data.parent_page.uid) {
    ...
    if (db) db.set(doc.default_uid, result)
    console.log(!!db, doc.default_uid, result)
    return result
  }
