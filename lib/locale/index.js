...
const { getPathById } = require('../resolve')
...
my18n.getAlternateLanguagePaths = function (alternateLanguages) {
  var langs = alternateLanguages.map(item => {
    return { 'name': my18n.getLocale(item.lang), 'url': getPathById(item.id) }
  })

  return langs
}

my18n.getLocale = function (locale) {
  switch (locale) {
    case 'de-de':
      return 'de'
    case 'en-gb':
