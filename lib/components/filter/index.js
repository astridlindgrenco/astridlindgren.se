    ...
    <div class="${classes}" data-container="filter" data-filter-active="${hasActiveFilter}">
      ${filterSection({
        ...
        body: hasActiveFilter
              ...
              : filterOptions(controls, requestQuery)
      }, formAction, isDisabled)}

      ${listOptions ? filterSorting(listOptions, formAction) : ''}
    </div>
  `
}

function filterOptions (controls, requestQuery) {
  return html`
        ...
        ${right
          ? html`
            <div class="Filter-sortRight u-flex u-lg-flexJustifyEnd u-lg-size1of2">
              ...
              ${createListOptions(right.groups)}
            </div>
          `
          : ''}
      </div>
 ...
 */
function createListOptions (groups) {
  return html`
