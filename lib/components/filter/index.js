...
function Filter ({
  ...
  if (!options || (documents && documents.length === 0)) return ''
  ...
  return html`
    <div class="${classes}" data-container="filter" data-filter-active="${hasActiveFilter}">
      ...
      ${listOptions ? filterSorting(listOptions, requestQuery, formAction) : ''}
    </div>
  `
}
/*
...
${
  listOptions
  ? filterSorting(Object.assign(...Object.entries(listOptions).map(function configureListOption ([position, config]) {
    console.log(config)
    return {
      [position]: {
        header: `${config.header}:`,
        body: createListOptions(config.items, requestQuery)
      }
    }
  })), formAction)
  : ''
...
function filterSorting (filter, requestQuery, formAction) {
        ...
        ${left
          ? html`
            <div class="Filter-sortLeft u-flex u-lg-flexJustifyStart u-lg-size1of2">
              ...
              ${createListOptions(left.groups, requestQuery)}
            </div>
          `
          : ''}
        ${right
          ? html`
            <div class="Filter-sortRight u-flex u-lg-flexJustifyEnd u-lg-size1of2">
              ...
              ${createListOptions(right.groups, requestQuery)}
            </div>
          `
          : ''}
      </div>
 ...
 */
function createListOptions (groups, requestQuery) {
  return html`
              ...
              return html`
                <div class="Filter-toggle">
                  ${choices.map(function createChoice (choice) {
                    ...
                    return html`
                      <input class="Filter-toggleInput" id="${choice.id}" type="radio" name="${choice.name}" value="${choice.value}"${raw(requestQuery[choice.name] === choice.value ? ' checked="checked"' : '')} onchange="this.form.submit()">
