/**
 * This is an excerpt of a proposal for components in choo
...
function ChooComponentCache (state, emit) {
  assert.strictEqual(typeof state, 'object', 'ChooComponentCache: state should be type object')
  assert.strictEqual(typeof emit, 'function', 'ChooComponentCache: state should be type function')
...
ChooComponentCache.prototype.render = function (Component) {
  assert.strictEqual(typeof Component, 'function', 'ChooComponentCache.render: Component should be type function')
  var args = []
  for (var i = 1, len = arguments.length; i < len; i++) {
    args.push(arguments[i])
  }

  assert.strictEqual(typeof Component.identity, 'function', 'ChooComponentCache.render: Component.identity should be type function')
  var id = Component.identity.apply(Component, args)
  assert.strictEqual(typeof id, 'string', 'ChooComponentCache.render: Component.identity should return type string')
