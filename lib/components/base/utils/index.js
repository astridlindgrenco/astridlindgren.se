...
/**
 * Prismic added IMGIX image optimization to their CMS as standard on 2019-10-28
 * Before this change, ImageKit was used for this project and all image URLs received from a Prismic document
 * was reformatted to fetch an optimized image from ImageKit instead of the raw image from Prismic (which can be very large)
 *
 * After this change, the Prismic document image URLs points to a new Prismic CDN which uses IMGIX behind the scenes
 * and the query string delimiter ? and the parameter auto=compress,format is tacked on by default by Prismic and cannot be disabled
 *
 * The below changes are meant to handle the transition period by detecting the old Prismic image source URL
 * and using the old reformatting but against the new IMGIX backend if so.
 * If that source URL is not detected, the new IMGIX-based URLs are created while expecting the ?auto=compress,format querystring to be present
 * */

const CDN_URL = 'https://images.prismic.io/astridlindgren/'
const OLD_SOURCE_URL = 'https://astridlindgren.cdn.prismic.io/astridlindgren/'
...
// Parse the Image URLs from a Prismic document and add optimization parameters for sizes
exports.image = function image (props, use = ['fw-small'], tr = '') {
  ...
  // Prismics new image CDN automatically compresses and optimizes images, so let's just add our sizes
  let src = `${props.url}&w=${width & width < 1440 ? width : 1440}`

  const oldImageName = props.url.split(OLD_SOURCE_URL)[1]

  // Old source URL detected, we need to add the entire querystring for Prismics new image CDN manually
  if (oldImageName) {
    src = `${CDN_URL}${oldImageName}?auto=compress,format&w=${width & width < 1440 ? width : 1440}`
  }
  ...
  const srcset = breakpoints.reduce((set, [name, width, sizes, quality]) => {
    return set.concat(sizes.map((size, index) => {
      if (oldImageName) {
        // Old source URL detected, we need to add the entire querystring for Prismics new image CDN manually
        return `${CDN_URL}${oldImageName}?auto=compress,format&w=${size} ${size}w`
      } else {
        return `${CDN_URL}&w=${size} ${size}w`
      }
    }))
  }, []).join(',')
...
exports.dividerImage = function dividerImage (url) {
  const oldImageName = url.split(OLD_SOURCE_URL)[1]

  // Old source URL detected, we need to add the entire querystring for Prismics new image CDN manually
  if (oldImageName) {
    return {
      srcset: `${CDN_URL}${oldImageName}?auto=compress,format&h=80&w=80&dpr=3 240w,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=80&w=80&dpr=2 160w,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=80&w=80&dpr=1 80w,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=60&w=60&dpr=3 180w,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=60&w=60&dpr=2 120w,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=60&w=60&dpr=1 60w`,
      sizes: '(min-width: 480px) 80px, 60px',
      src: `${CDN_URL}${oldImageName}?auto=compress,format&h=80&w=80&dpr=1`
    }
  }

  return {
    srcset: `${CDN_URL}&h=80&w=80&dpr=3 240w,
              ${CDN_URL}&h=80&w=80&dpr=2 160w,
              ${CDN_URL}&h=80&w=80&dpr=1 80w,
              ${CDN_URL}&h=60&w=60&dpr=3 180w,
              ${CDN_URL}&h=60&w=60&dpr=2 120w,
              ${CDN_URL}&h=60&w=60&dpr=1 60w`,
    sizes: '(min-width: 480px) 80px, 60px',
    src: `${CDN_URL}&h=80&w=80&dpr=1`
  }
}

exports.characterImage = function characterImage (url) {
  const oldImageName = url.split(OLD_SOURCE_URL)[1]

  // Old source URL detected, we need to add the entire querystring for Prismics new image CDN manually
  if (oldImageName) {
    return {
      srcset: `${CDN_URL}${oldImageName}?auto=compress,format&h=300&w=300&dpr=3 3x,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=300&w=300&dpr=2 2x,
               ${CDN_URL}${oldImageName}?auto=compress,format&h=300&w=300&dpr=1 1x`,
      src: `${CDN_URL}${oldImageName}?auto=compress,format&h=300&w=300&dpr=1`
    }
  }

  return {
    srcset: `${CDN_URL}&h=300&w=300&dpr=3 3x,
             ${CDN_URL}&h=300&w=300&dpr=2 2x,
             ${CDN_URL}&h=300&w=300&dpr=1 1x`,
    src: `${CDN_URL}&h=300&w=300&dpr=1`
  }
}
