...
const SIZES = [
  ['fw-small', 0, [400, 800], [80, 40]],
  ['hw-small', 0, [200, 400], [80, 40]],
  ['qw-small', 0, [100, 200], [80, 40]],
  ['fw-medium', 400, [860, 1720], [80, 40]],
  ['hw-medium', 400, [430, 860], [80, 40]],
  ['qw-medium', 400, [215, 430], [80, 40]],
  ['fw-large', 860, [1100, 2200], [80, 40]],
  ['hw-large', 860, [550, 1100], [80, 40]],
  ['qw-large', 860, [225, 550], [80, 40]],
  ['fw-xlarge', 1100, [1440, 2880], [80, 40]],
  ['hw-xlarge', 1100, [720, 1440], [80, 40]],
  ['qw-xlarge', 1100, [360, 720], [80, 40]]
]
...
exports.image = function image (props, use = ['fw-small']) {
  ...
  const srcset = breakpoints.reduce((set, [name, width, sizes, quality]) => {
    return set.concat(sizes.map((size, index) => {
      return `${CDN_URL}tr:w-${size},q-${quality[index]},pr-true/${uri} ${size}w`
    }))
  }, []).join(',')
...
exports.icon = function icon (url) {
  const uri = url.split(SOURCE_URL)[1]
  return {
    srcset: `${CDN_URL}tr:h-80,w-80,c-at_max,dpr-4.0,t-true,f-png/${uri} 4x,
             ${CDN_URL}tr:h-80,w-80,c-at_max,dpr-2.5,t-true,f-png/${uri} 2.5x,
             ${CDN_URL}tr:h-80,w-80,c-at_max,dpr-1.6,t-true,f-png/${uri} 1.6x,
             ${CDN_URL}tr:h-80,w-80,c-at_max,dpr-1.0,t-true,f-png/${uri} 1x`,
    src: `${CDN_URL}tr:h-80,w-80,c-at_max,dpr-1.0,t-true,f-png/${uri}`
  }
}
