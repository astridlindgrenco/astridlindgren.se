...
function links (slice, state, container) {
  ...
  if (data.intro_text.length || data.intro_title.length) {
    header.push(html`
      <div class="Display-title">${data.intro_title ? asElement(data.intro_title, linkResolver, serializer({
        classes: {
          heading2: {
            [getCustomFontClass(state.font)]: state && state.font
          }
        }
      })) : ''}</div>
  ...
  if (data.more_label) {
    footer.push(html`<div class="Text"><p><a href="${Link.url(data.more_link, linkResolver)}"">${data.more_label}</a></p></div>`)
  }
  return Section({
    push: true,
