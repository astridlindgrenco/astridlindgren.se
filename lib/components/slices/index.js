...
function slices (state, body, container = Section) {
  ...
  return body.map(function renderSlice (slice) {
    switch (slice.slice_type) {
      ...
      */
      case 'bokhylla': {
        let data = slice.primary
        let itemsFromState = getLinkedDocsInSlices(slice.items)
        return Section({
          push: true,
          isFill: true,
          bg: data.theme_section_bg_color,
          header: data.bookshelf_title
                  ? html`<div class="Text">${asElement(data.bookshelf_title)}</div>`
                  : false,
          footer: data.link_source
                  ? html`<div class="Text"><p><a href="${Link.url(data.link_source, linkResolver)}}">${data.link_label}</a></p></div>`
                  : false,
          isFillLarge: isFullWidth,
          body: Display(state, itemsFromState.map(function crateDisplayItem (item) {
            let title = item.data.book_title[0].text
            return {
              type: item.type,
              cover: html`
                <a href="/${item.uid}">
   ...
   */
  function getLinkedDocsInSlices (slices) {
