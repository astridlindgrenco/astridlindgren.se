  ...
  return Section({
    ...
    header: html`
      ...
      ${data.bookshelf_title ? html`
        <div class="Display-title${(state && state.font) ? ' ' + FontClass(state.font) : ''}">${asElement(data.bookshelf_title)}</div>` : ''}
        ${data.titel ? html`<div class="Display-title">${asElement(data.titel)}</div>` : ''}
    `,
    footer: html`
      ${data.more_label ? html`<div class="Text"><p><a href="${data.more_link && data.more_link.url ? data.more_link.url : linkResolver(data.more_link)}">${data.more_label}</a></p></div>` : ''}
    `,
