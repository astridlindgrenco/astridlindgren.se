const url = require('url')
const html = require('nanohtml')
const { Elements } = require('prismic-richtext')
const linkResolver = require('../../resolve')

module.exports = serialize

function serialize (type, element, content, children) {
  switch (element.type) {
    case Elements.paragraph: return html`<p>${children}</p>`
    case Elements.heading1: return html`<h1 id="${id(element.text)}">${children}</h1>`
    case Elements.heading2: return html`<h2 id="${id(element.text)}">${children}</h2>`
    case Elements.heading3: return html`<h3 id="${id(element.text)}">${children}</h3>`
    case Elements.heading4: return html`<h4 id="${id(element.text)}">${children}</h4>`
    case Elements.heading5: return html`<h5 id="${id(element.text)}">${children}</h5>`
    case Elements.heading6: return html`<h6 id="${id(element.text)}">${children}</h6>`
    case Elements.hyperlink: {
      if (element.data.target && element.data.target === '_blank') {
        return html`<a href="${resolve(element.data)}" target="_blank" rel="noopener noreferrer">${children}</a>`
      }
      return html`<a href="${resolve(element.data)}">${children}</a>`
    }
    default: return null
  }
}

/**
 * Resolve link fixing Prismic adding protocol to relative links and anchors
 *
 * @param {obj} link
 * @returns str
 */

function resolve (link) {
  if (link.link_type === 'Document') return linkResolver(link)
  const { hostname, pathname, hash } = url.parse(link.url)
  if (!hostname) return (pathname || '') + (hash || '')
  if (hostname.indexOf('.') === -1) return '/' + hostname + (pathname || '')
  return link.url
}

/**
 * Generate url friendly string from text
 * @param {str} text
 */

function id (text) {
  return text.replace(/[^\w]/g, '-').replace(/-{2,}/, '-').toLowerCase()
}
