    ...
    switch (element.type) {
      ...
      case Elements.hyperlink: {
        if (element.data.target && element.data.target === '_blank') {
          return html`<a ${raw(setClass('hyperlink'))} href="${resolve(element.data)}" target="_blank" rel="noopener noreferrer">${children}</a>`
        }
        return html`<a ${raw(setClass('hyperlink'))} href="${resolve(element.data)}">${children}</a>`
      }
      case Elements.embed: {
