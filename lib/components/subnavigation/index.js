...
function subNavigation (state, context) {
  ...
  try {
    ...
    if (state.subNav.some(hasLabel)) {
      return html`
        ${state.is_character_menu ? raw('<div class="SubNav-CharacterMenu-background" style="background-color:' + palette.getHexCode(state.character_menu.accent) + ';"></div>') : null}
        <div id="sub-menu" class="${state.is_character_menu ? 'SubNav-CharacterMenu' : 'SubNav'}"
          style="${state.character_menu ? ('background-color: ' + palette.getHexCode(state.character_menu.accent) + ';') : ''}">
          ${raw(state.is_character_menu ? `<div class="SubNav-CharacterMenu-Title${state.font ? ' ' + FontClass(state.font) : ''}"><a class="SubNav-title" href="#sub-menu">` + state.character_menu.title + '<img class="Subnav-toggle-icon" src="/character_menu_icon.svg"/></a><a class="SubNav-close" href="#">' + state.character_menu.title + '<img class="Subnav-toggle-icon Subnav-toggle-icon--close" src="/character_menu_icon.svg"/></a></div>' : '')}
