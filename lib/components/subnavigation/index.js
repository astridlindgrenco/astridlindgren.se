    ...
    if (state.subNav.some(hasLabel)) {
      return html`
        ...
        ${raw(`<div class="${state.is_character_menu && !parentMenu ? 'SubNav-CharacterMenu' : 'SubNav'}"
          ...
          ${raw(state.is_character_menu && !parentMenu ? `<div class="SubNav-CharacterMenu-Title${state.font ? ' ' + FontClass(state.font) : ''}"><a class="SubNav-title" onclick="toggleMenu();">` + state.character_menu.title + '<img class="Subnav-toggle-icon" src="/character_menu_icon.svg"/></a></div>' : '')}
