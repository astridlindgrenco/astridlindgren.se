    ...
    if (state.subNav.some(hasLabel)) {
      return html`
        ...
        <div id="sub-menu" class="${state.is_character_menu ? 'SubNav-CharacterMenu' : 'SubNav'}"
          ...
          ${raw(state.is_character_menu ? `<div class="SubNav-CharacterMenu-Title${state.font ? ' ' + FontClass(state.font) : ''}"><a class="SubNav-title" href="#sub-menu">` + state.character_menu.title + '<span class="Subnav-toggle-icon">&#709;</span></a><a class="SubNav-close" href="#">' + state.character_menu.title + '<span class="Subnav-toggle-icon">&#708;</span></a></div>' : '')}
