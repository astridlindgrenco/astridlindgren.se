...
module.exports = function page (state) {
  ...
  // FIXME: temporary error page, split for 404 and all others
  // should not end up here anyway...
  if (page.error) {
  ...
  const heroImage = doc.data.hero_image
  const text = html(DOM.RichText.asHtml(doc.data.main_body))
