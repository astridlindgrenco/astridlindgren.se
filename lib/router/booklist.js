...
var bookFilters = {
  ...
  'character': function characterFilter (value) {
    return {
      predicates: [Prismic.Predicates.has(`my.book.character`)],
      postFilter: function onlySpecificCharacter (response) {
        response.results = response.results.filter(doc => doc.data.character.id === value)
        return response
      }
    }
  }
...
router.get(`/:langCode/${__('booklist')}`,
  async function (ctx, next) {
    ...
    // get options
    const orderings = getOrderings(ctx.request.query, doc.data.default_sort_by)

    // get predicates
    const queryOptions = getFilterOptions(ctx.request.query)
    ...
    ctx.state.ui.bookFilter = {
      viewBy: getViewBy(ctx.request.query, doc.data.default_view),
      sortBy: getSortBy(ctx.request.query, doc.data.default_sort_by),
...
function getFilterOptions (query) {
  var options = Array.prototype.concat(...Object.entries(query).map(pair => getFilter(...pair)))
  return {
    predicates: options.map(filter => filter.predicates),
    postFilters: pipe(...options.map(filter => filter.postFilter).filter(Boolean))
  }
}
...
function getFilter (name, value) {
  ...
  var constructFilter = bookFilters[name]
  if (typeof constructFilter === 'function') {
    return constructFilter(value)
  }
}
