...
router.get(`/:locale/${__('the works')}/${UIDS.books}`, listpage(UIDS.books))
router.get(`/:locale/${__('the works')}/${UIDS.movies}`, listpage(UIDS.movies))

function listpage (uid) {
  return async function listpage (ctx, next) {
    ctx.state.uid = uid
    ctx.view = 'listpage'
    const doc = await ctx.prismic.getByUID('listpage', uid, { lang: ctx.state.lang })
    ...
    // Determine type of Prismic document
