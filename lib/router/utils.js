...
/**
 * Find out which tem in primary and secondary header menu that should marked.
...
async function updateNavigation (doc, ctx, next) {
  let id = doc.data.parent_page.id || doc.id
  // mark menu item
  markMenuItem(id, ctx.state.navDocument.data.primary_links)
  markMenuItem(id, ctx.state.navDocument.data.secondary_links)
}

function markMenuItem (id, links) {
  links.forEach(link => {
    link.isActive = link.source.id === id
    console.log(id, link.source.id, link.isActive)
  })
}
