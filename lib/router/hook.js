...
router.all('*',
  async function (ctx, next) {
    ...
    // clean URL from WS
    ctx.url = ctx.url.replace(/^[\w\.\+\-\&\?\=]|%0\d|%1\d/g, '') // eslint-disable-line
...
async function emptyCache () {
  if (process.env.VARNISH_IP) {
    axios({
      ...
      console.log(`[Hook]: Empty cache ${process.env.VARNISH_IP} returned ${response.status}.`)
    }).catch(function (error) {
      console.log(`[Hook]: Empty cache ${process.env.VARNISH_IP} returned ${error}.`)
    })
  } else {
    console.error('missing VARNISH_IP (.env)')
  }
}
